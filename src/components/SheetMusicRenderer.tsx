...
const SheetMusicRenderer = ({ musicXmlBase64, instrument, transposeSemitones = 0 }: SheetMusicRendererProps) => {
  ...
  if (!musicXmlBase64) {
    return (
      <div className="flex flex-col items-center justify-center h-48 bg-paper border border-border rounded-lg gap-3">
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-ink">
          <path d="M9 18V5l12-2v13" />
          <circle cx="6" cy="18" r="3" />
          <circle cx="18" cy="16" r="3" />
        </svg>
        <div className="text-center">
          <p className="text-sm font-medium text-ink">No transcription found for this instrument</p>
          <p className="text-xs text-muted-foreground mt-1">Try selecting a different instrument tab above.</p>
        </div>
      </div>
    );
  }
